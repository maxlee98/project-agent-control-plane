...
export async function POST(request: Request, { params }: { params: Promise<{ runId: string }> }) {
  try {
    ...
    let run;
    try {
      run = startAgentRun(sourceRun.taskId, "retry", runId, reasoningEffort as ReasoningEffort | null | undefined);
    } catch {
    ...
    if (!run) {
      const payload = { code: "RUN_CONFLICT", message: "This run could not be retried." };
    ...
    return apiErrorFrom(error, "The run retry request could not be completed.");
  }
}