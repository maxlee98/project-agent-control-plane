...
import { normalizeTaskStatus, type Project, type Task, type TaskStatus } from "@/lib/domain";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ taskId: string }> }) {
  ...
  const body = await request.json() as { status?: TaskStatus | "agent_review" | "in_review" | "review"; priority?: number; title?: string; description?: string; estimatedCostUsd?: unknown; comment?: string };
  const requestedStatus = body.status === undefined ? undefined : normalizeTaskStatus(body.status);
  ...
  if (requestedStatus === "done") {
    ...
    if (process.env.EXECUTION_MODE === "live") {
      try { remote = await syncTaskStatus(currentProject, currentTask, requestedStatus); }
  ...
  const task = updateTask(taskId, { status: requestedStatus, priority: body.priority, title: body.title, description: body.description, estimatedCostCents });
  return task ? Response.json({ ...task, remoteSync: remote ?? null }) : Response.json({ error: "Task not found." }, { status: 404 });
}