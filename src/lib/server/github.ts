...
export async function createPullRequest(fullName: string, task: Task, run: AgentRun) {
  ...
  const { owner, repo } = repoParts(fullName);
  const body = await readResponse(await githubRequest(`/repos/${owner}/${repo}/pulls`, { method: "POST", body: JSON.stringify({ title: task.title, head: run.branchName, base: process.env.GITHUB_DEFAULT_BRANCH ?? "main", body: `Fixes #${task.issueNumber}\n\n${task.currentSummary}\n\nCommit: ${run.commitSha ?? "not recorded"}` }) }));
