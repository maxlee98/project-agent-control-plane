...
- Follow-up recommendations: at most 5 proposals per request; titles are limited to 200 characters,
  rationales to 2,000, descriptions to 6,000, and acceptance criteria to 8 items of 500 characters.
...
task comments, task status changes, follow-up recommendation, and follow-up creation. The key is scoped to
the operation and request fingerprint in the local SQLite database.
...
`POST /api/tasks/:taskId/follow-ups/recommend` is available only for Done and Review tasks. It returns a
bounded, redacted proposal list for operator review and records the recommendation outcome in activity.
Demo mode uses a deterministic local fallback and does not call a provider or GitHub.

`POST /api/tasks/:taskId/follow-ups/create` accepts the selected, edited proposal list and requires an
`Idempotency-Key`. Every proposal is reported independently. Demo mode creates only local Inbox tasks and
returns `noGithubRequest: true`; Live mode creates canonical Issues, synchronizes each Issue to the
configured Projects V2 board, and stores the resulting Issue/task links. A `207` response indicates a
partial result and failed proposals are never reported as created.